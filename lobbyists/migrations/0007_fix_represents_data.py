...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        """
        go over all the lobbyist represents data and remove any which don't include any changes in them
        """
        objs_to_delete = []
        all_lobbyist_represents = {}
        prev_lobbyist_represents_datas = {}
        for lobbyist_represents_data in orm.LobbyistRepresentData.objects.order_by('scrape_time'):
            if lobbyist_represents_data.name not in all_lobbyist_represents:
                all_lobbyist_represents[lobbyist_represents_data.name] = lobbyist_represents_data.lobbyist_represent
            lobbyist_represents = all_lobbyist_represents[lobbyist_represents_data.name]
            if lobbyist_represents != lobbyist_represents_data.lobbyist_represent:
                old_lobbyist_represents = lobbyist_represents_data.lobbyist_represent
                objs_to_delete.append(old_lobbyist_represents)
                # replace old lobbyist represents with the new one
                for lobbyist_data in orm.LobbyistData.objects.filter(represents__in=[old_lobbyist_represents]):
                    lobbyist_data.represents.remove(old_lobbyist_represents)
                    lobbyist_data.represents.add(lobbyist_represents)
                lobbyist_represents_data.lobbyist_represent = lobbyist_represents
                lobbyist_represents_data.save()
            if lobbyist_represents.name in prev_lobbyist_represents_datas:
                prev_lobbyist_represents_data = prev_lobbyist_represents_datas[lobbyist_represents.name]
                ok_to_delete = True
                for field in ['name', 'domain', 'type']:
                    if getattr(prev_lobbyist_represents_data, field) != getattr(lobbyist_represents_data, field):
                        ok_to_delete = False
                if ok_to_delete:
                    objs_to_delete.append(lobbyist_represents_data)
            prev_lobbyist_represents_datas[lobbyist_represents.name] = lobbyist_represents_data
        for lobbyist_represents in orm['lobbyists.LobbyistRepresent'].objects.all():
            if lobbyist_represents.name in all_lobbyist_represents and all_lobbyist_represents[lobbyist_represents.name] != lobbyist_represents:
                objs_to_delete.append(lobbyist_represents)
    ...
    def backwards(self, orm):
        """
        we can't undo this operation but it doesn't really matter
        """
        pass
